...
SPECIALTIES_FILE = BASE_DIR / "data" / "specialties.json"
IMAGE_MANIFEST_FILE = BASE_DIR / "data" / "image-manifest.json"
_specialties_cache: list[dict] | None = None
_image_manifest_cache: dict | None = None
...
def load_specialties() -> list[dict]:
    global _specialties_cache
    if _specialties_cache is None:
        _specialties_cache = json.loads(SPECIALTIES_FILE.read_text(encoding='utf-8'))
    return _specialties_cache

def specialty_for(name: str) -> dict:
    wanted = name.strip().casefold()
    return next((x for x in load_specialties() if x['name'].casefold() == wanted), {})



def load_image_manifest() -> dict:
    global _image_manifest_cache
    if _image_manifest_cache is None:
        _image_manifest_cache = json.loads(IMAGE_MANIFEST_FILE.read_text(encoding="utf-8"))
    return _image_manifest_cache

def lesson_visual(lesson_id: int, module: str) -> dict:
    manifest = load_image_manifest()
    item = manifest.get("lessons", {}).get(str(lesson_id), {})
    spec = specialty_for(module)
    return {
        "image": item.get("image") or spec.get("image", ""),
        "image_alt": item.get("alt") or spec.get("alt", module),
        "accent": item.get("accent") or spec.get("accent", "#61E1D0"),
        "specialty_id": item.get("specialty_id") or spec.get("id", ""),
    }
    ...
    items = [
        {
            ...
            "image": specialty_for(name).get("image", ""),
            "accent": specialty_for(name).get("accent", "#61E1D0"),
            "alt": specialty_for(name).get("alt", name),
            "id": specialty_for(name).get("id", ""),
        }
...
@app.get("/api/images")
def api_images():
    return jsonify(load_image_manifest())
