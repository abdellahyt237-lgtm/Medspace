(() => {
  ...
  function chooseModule(name) { state.module=name; renderModuleChips(); fetchLessons(true); document.querySelector('#library').scrollIntoView({behavior:'smooth', block:'start'}); }
  ...
  async function openReader(lesson) {
    ...
    $('#readerTitle').textContent=lesson.title; $('#readerModule').textContent=lesson.module; $('#readerIcon').innerHTML=icon(lesson.icon); $('#readerPosition').textContent='جاري تحديد الموقع…';
  ...
  function bind(){
    ...
    $('#continueBtn').onclick=continueLast;
